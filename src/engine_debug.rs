...
impl EngineDebugApp{
    ...
    async fn process(&mut self, mouse_world : Vec2){
        ...
        if let Some(t) = self.selected_tile{
            t.draw_highlight_outline(0.2, GREEN, self.flip_board);
        ...
        if let Some(selected_tile) = self.hovered_tile{
            selected_tile.draw_highlight_outline(0.1,GRAY,self.flip_board);
        }
...
pub async fn main() {
    let mut state = EngineDebugApp::new().await;
